...
Question: Max-Product Composition of two Fuzzy Relations
"""

def max_product_composition(R, S, m, n, p):
    T = [[0 for _ in range(p)] for _ in range(m)]
    for i in range(m):
        for j in range(p):
            values = []
            for k in range(n):
                values.append(R[i][k] * S[k][j]) #Formula: C = A ∘ B, Cᵢⱼ = maxₖ { aᵢₖ × bₖⱼ }
...
T = max_product_composition(R, S, m, n, p)

print_matrix("Relation R (X x Y)", R)
print_matrix("Relation S (Y x Z)", S)
print_matrix("Composition T = R∘S (X x Z) [Max-Product]", T)
...
Composition T = R∘S (X x Z) [Max-Product]:
[0.42, 0.63]
[0.4, 0.72]
'''